...
public class Estrategias {
		...
		while (!cola.isEmpty()) {
			...
			for (int i = 0; i < g.length; i++) {
				if (g[j][i] == 1 && !visitiadoProfunidad[i]) {
					cola.add(i);
					recorridos.addAll(recorridoProfunidad(i)); // Se recorren los hijos del nodo actual de visita y se
																// agrega el recorrido a la lista
	...
	public ArrayList<Nodo> busquedaCosteUniforme(Nodo nodoInicial) {
		...
		while(!nodosFrontera.isEmpty()) {
			nodosFrontera.//ordenar segun coste SUCESORES
