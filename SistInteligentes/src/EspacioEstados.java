...
public class EspacioEstados {
	...
	public Estado crearEstado(ArrayList<int[]> distribuciones, Acciones accion, Estado est) {
		...
		for (int i = 0; i < distribuciones.size(); i++) {
			nuevo[distribuciones.get(i)[0]][distribuciones.get(i)[1]] = est.getTerreno()
					.getCantidad(distribuciones.get(i)[0], distribuciones.get(i)[1]) + accion.getDist()[i];
		}

		if (nuevo[est.getTractorX()][est.getTractorY()] > est.getTerreno().K())
			nuevo[est.getTractorX()][est.getTractorY()] = est.getTerreno().K();

		return new Estado(new Terreno(est.getTerreno().K(), est.getTerreno().Max(), nuevo), accion.getMov()[0],
				accion.getMov()[1]);
	}
	...
	public ArrayList<Sucesor> getSucesores(Estado est) {
		...
		posiciones = back(new int[distribuciones.size()], 0, posiciones, cant, distribuciones.size(), distribuciones,
				est);
		...
		for (int k = 0; k < candidatos.size(); k++)
			sucesores.add(
					new Sucesor("acc" + k, candidatos.get(k), crearEstado(distribuciones, candidatos.get(k), est)));
	...
	private boolean esPosible(int[] is, ArrayList<int[]> distribuciones, Estado est) {
		...
		for (int i = 0; i < is.length; i++) {
			if (est.getTerreno().getTerreno()[distribuciones.get(i)[0]][distribuciones.get(i)[1]] + is[i] > est
					.getTerreno().Max())
				flag = false;
		}
		return flag;
	...
	public static ArrayList<int[]> crearLista(Estado est) {
		...
		/*
		 * Si el tractor esta en una esquina del terreno, tiene dos casillas donde poder
		 * colocar tierra
		 */
		...
		} else if (est.getTractorX() == 0 && est.getTractorY() == est.getTerreno().getFilas() - 1) { // Esquina inferior
																										// izquierda
			lista.add(new int[] { (est.getTractorX() + 1), est.getTractorY() });
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() - 1) });

		} else if (est.getTractorX() == est.getTerreno().getColumnas() - 1 && est.getTractorY() == 0) { // Esquina
																										// superior
																										// derecha
			lista.add(new int[] { (est.getTractorX() - 1), est.getTractorY() });
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() + 1) });

		} else if (est.getTractorX() == est.getTerreno().getColumnas() - 1
				&& est.getTractorY() == est.getTerreno().getFilas() - 1) { // Esquina inferior derecha
			lista.add(new int[] { (est.getTractorX() - 1), est.getTractorY() });
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() - 1) });
		}

		/*
		 * Si el tractor esta en el borde izquierdo, pero no en una esquina. Tiene tres
		 * casillas donde poder colocar tierra
		 */

		else if (est.getTractorX() == 0 && est.getTractorY() != 0
				&& est.getTractorY() != est.getTerreno().getFilas() - 1) { // Lateral
			// izquierdo
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() - 1) });
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() + 1) });
			lista.add(new int[] { (est.getTractorX() + 1), est.getTractorY() });
			...
			/*
			 * Si el tractor esta en el borde derecho, pero no en una esquina. Tiene tres
			 * casillas donde poder colocar tierra
			 */

		} else if (est.getTractorX() == est.getTerreno().getColumnas() - 1 && est.getTractorY() != 0
				&& est.getTractorY() != est.getTerreno().getFilas() - 1) { // Lateral derecho
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() - 1) });
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() + 1) });
			lista.add(new int[] { (est.getTractorX() - 1), est.getTractorY() });

			/*
			 * Si el tractor esta en el borde de arriba, pero no en una esquina. Tiene tres
			 * casillas donde poder colocar tierra
			 */

		} else if (est.getTractorY() == 0 && est.getTractorX() != 0
				&& est.getTractorX() != est.getTerreno().getColumnas() - 1) { // Arriba
			lista.add(new int[] { (est.getTractorX() - 1), est.getTractorY() });
			lista.add(new int[] { (est.getTractorX() + 1), est.getTractorY() });
			lista.add(new int[] { est.getTractorX(), (est.getTractorY() + 1) });

			/*
			 * Si el tractor esta en el borde de abajo, pero no en una esquina. Tiene tres
			 * casillas donde poder colocar tierra
			 */
			...
			/*
			 * Si el tractor esta en la mitad del terreno, tiene cuatro casillas donde poder
			 * colocar tierra
			 */
	...
	private ArrayList<int[]> back(int[] actual, int etapa, ArrayList<int[]> sol, int max, int ncasillas,
			ArrayList<int[]> distribuciones, Estado est) {
